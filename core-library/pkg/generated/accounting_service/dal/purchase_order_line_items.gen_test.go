...
func init() {
	InitializeDB()
	err := db.AutoMigrate(&accounting_servicev1.PurchaseOrderLineItemORM{})
	if err != nil {
		fmt.Printf("Error: AutoMigrate(&accounting_servicev1.PurchaseOrderLineItemORM{}) fail: %s", err)
	}
}
...
func Test_purchaseOrderLineItemORMQuery(t *testing.T) {
	...
	err = _do.Create(&accounting_servicev1.PurchaseOrderLineItemORM{})
	if err != nil {
		t.Error("create item in table <purchase_order_line_items> fail:", err)
	}

	err = _do.Save(&accounting_servicev1.PurchaseOrderLineItemORM{})
	if err != nil {
		t.Error("create item in table <purchase_order_line_items> fail:", err)
	}

	err = _do.CreateInBatches([]*accounting_servicev1.PurchaseOrderLineItemORM{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <purchase_order_line_items> fail:", err)
	}
	...
	err = _do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*accounting_servicev1.PurchaseOrderLineItemORM{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
	...
	_, err = _do.ScanByPage(&accounting_servicev1.PurchaseOrderLineItemORM{}, 0, 1)
	if err != nil {
...
func Test_purchaseOrderLineItemORM_CreateRecord(t *testing.T) {
	...
	for i, tt := range PurchaseOrderLineItemORMCreateRecordTestCase {
		t.Run("CreateRecord_"+strconv.Itoa(i), func(t *testing.T) {
			res1 := do.CreateRecord(tt.Input.Args[0].(accounting_servicev1.PurchaseOrderLineItemORM))
...
func Test_purchaseOrderLineItemORM_UpdateRecordByID(t *testing.T) {
	...
	for i, tt := range PurchaseOrderLineItemORMUpdateRecordByIDTestCase {
		t.Run("UpdateRecordByID_"+strconv.Itoa(i), func(t *testing.T) {
			res1 := do.UpdateRecordByID(tt.Input.Args[0].(int), tt.Input.Args[1].(accounting_servicev1.PurchaseOrderLineItemORM))
...
var PurchaseOrderLineItemORMGetByIDTestCase = []TestCase{}

func Test_purchaseOrderLineItemORM_GetByID(t *testing.T) {
	purchaseOrderLineItemORM := newPurchaseOrderLineItemORM(db)
	do := purchaseOrderLineItemORM.WithContext(context.Background()).Debug()

	for i, tt := range PurchaseOrderLineItemORMGetByIDTestCase {
...
var PurchaseOrderLineItemORMGetByIDsTestCase = []TestCase{}

func Test_purchaseOrderLineItemORM_GetByIDs(t *testing.T) {
	purchaseOrderLineItemORM := newPurchaseOrderLineItemORM(db)
	do := purchaseOrderLineItemORM.WithContext(context.Background()).Debug()

	for i, tt := range PurchaseOrderLineItemORMGetByIDsTestCase {
