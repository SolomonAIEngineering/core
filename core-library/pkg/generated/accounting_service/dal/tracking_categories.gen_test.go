...
func init() {
	InitializeDB()
	err := db.AutoMigrate(&accounting_servicev1.TrackingCategoryORM{})
	if err != nil {
		fmt.Printf("Error: AutoMigrate(&accounting_servicev1.TrackingCategoryORM{}) fail: %s", err)
	}
}
...
func Test_trackingCategoryORMQuery(t *testing.T) {
	...
	err = _do.Create(&accounting_servicev1.TrackingCategoryORM{})
	if err != nil {
		t.Error("create item in table <tracking_categories> fail:", err)
	}

	err = _do.Save(&accounting_servicev1.TrackingCategoryORM{})
	if err != nil {
		t.Error("create item in table <tracking_categories> fail:", err)
	}

	err = _do.CreateInBatches([]*accounting_servicev1.TrackingCategoryORM{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <tracking_categories> fail:", err)
	}
	...
	err = _do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*accounting_servicev1.TrackingCategoryORM{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
	...
	_, err = _do.ScanByPage(&accounting_servicev1.TrackingCategoryORM{}, 0, 1)
	if err != nil {
...
func Test_trackingCategoryORM_CreateRecord(t *testing.T) {
	...
	for i, tt := range TrackingCategoryORMCreateRecordTestCase {
		t.Run("CreateRecord_"+strconv.Itoa(i), func(t *testing.T) {
			res1 := do.CreateRecord(tt.Input.Args[0].(accounting_servicev1.TrackingCategoryORM))
...
func Test_trackingCategoryORM_UpdateRecordByID(t *testing.T) {
	...
	for i, tt := range TrackingCategoryORMUpdateRecordByIDTestCase {
		t.Run("UpdateRecordByID_"+strconv.Itoa(i), func(t *testing.T) {
			res1 := do.UpdateRecordByID(tt.Input.Args[0].(int), tt.Input.Args[1].(accounting_servicev1.TrackingCategoryORM))
...
var TrackingCategoryORMGetByIDTestCase = []TestCase{}

func Test_trackingCategoryORM_GetByID(t *testing.T) {
	trackingCategoryORM := newTrackingCategoryORM(db)
	do := trackingCategoryORM.WithContext(context.Background()).Debug()

	for i, tt := range TrackingCategoryORMGetByIDTestCase {
...
var TrackingCategoryORMGetByIDsTestCase = []TestCase{}

func Test_trackingCategoryORM_GetByIDs(t *testing.T) {
	trackingCategoryORM := newTrackingCategoryORM(db)
	do := trackingCategoryORM.WithContext(context.Background()).Debug()

	for i, tt := range TrackingCategoryORMGetByIDsTestCase {
