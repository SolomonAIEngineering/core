...
func (m *Account) ToORM(ctx context.Context) (AccountORM, error) {
	...
	to.BucketLocation = m.BucketLocation
	to.Region = m.Region
...
type WorkspaceORM struct {
	...
	S3Region         string
	Tags             pq.StringArray `gorm:"type:text[]"`
	UniqueIdentifier string         `gorm:"unique_index:idx_workspace_unique_identifier"`
	UpdatedAt        *time.Time
	Version          int32
	VersionId        string
}
...
func DefaultApplyFieldMaskAccount(ctx context.Context, patchee *Account, patcher *Account, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Account, error) {
	...
	for _, f := range updateMask.Paths {
		...
		if f == prefix+"BucketLocation" {
			patchee.BucketLocation = patcher.BucketLocation
			continue
		}
		if f == prefix+"Region" {
